...
\textbf{AI Interview Guidelines}\\
{\LARGE Machine Learning and Deep Learning}\par % Book title
\vspace*{1cm}
{\LARGE Ron Korine, 2024}\par % Author name
\endgroup
...
\noindent \textit{First release, December 2024} % Printing/edition date
...
Performance metrics are crucial to evaluate the performance of any machine learning model. This applies to all types of machine learning models, whether the model is simple like linear regression, and whether it is a complex SOTA model (e.g., a large LLM). 

Hence, to evaluate candidate machine learning practitioners, it is crucial to see if they are familiar with the basic performance metrics and understand which metrics are the most fitting for the task at hand. 

A machine learning practitioner who chooses the right ways to evaluate machine learning models is on the right track to solve the task, or at the very least, to  

Every machine learning task can be broken down into Regression or Classification, and accordingly, so are performance metrics. 
