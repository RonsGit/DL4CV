...
\begin{enrichment}[Self-Supervised Video Pretraining for VLLMs][section]
	...
	\begin{enrichment}[MVD: Masked Video Distillation][subsection]
		\label{enr:subsec_chapter24_mvd}
			...
			On SSv2, MVD dominates the accuracy–compute frontier relative to supervised and self-supervised peers (Fig.~\ref{fig:chapter24_mvd_flops}). Relative to VideoMAE \cite{tong2022_videomae}, MVD delivers consistent gains across model scales with substantially fewer pretraining epochs \cite{wang2023_mvd}.
			...
			\begin{table}[H]
				...
				\begin{tabular}{lcccc}
					\toprule
					\textbf{Student} & \textbf{Image teacher} & \textbf{Video teacher} & \textbf{K400 top-1 (\%)} & \textbf{SSv2 top-1 (\%)} \\
					\midrule
					ViT-S & \cmark & \xmark & 80.4 & 69.4 \\
					ViT-S & \xmark & \cmark & 80.1 & 70.0 \\
					ViT-S & \cmark & \cmark & \textbf{80.6} & \textbf{70.7} \\
