...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_2/Slide_27.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_2/Slide_31.jpg}
...
The \textbf{CIFAR-10} dataset \cite{krizhevsky2009_learning} represents a significant step forward in dataset complexity. It contains 10 classes of objects (e.g., airplane, automobile, bird, cat) with $32 \times 32 \times 3$ RGB images. The dataset includes 50,000 training images (5,000 per class) and 10,000 test images (1,000 per class). CIFAR-10 strikes a balance between complexity and computational feasibility, making it ideal for research and teaching purposes. In this course, CIFAR-10 is the primary dataset for homework assignments.
...
\begin{itemize}
	...
	\begin{figure}[H]
		...
		\caption{Comparison of L1 and L2 distance metrics.}
...
\begin{figure}[H]
	...
	\caption{Decision boundaries produced by different distance metrics (L1 vs. L2).}
	\label{fig:chapter2_l1_l2_comparison}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_2/Slide_65.jpg}
	\caption{Nearest Neighbor using TF-IDF similarity for academic paper recommendations.}
	\label{fig:chapter2_nn_tfidf}
\end{figure}

Using TF-IDF scores, we can represent each academic paper as a feature vector. Nearest Neighbor can then be employed to find similar papers by comparing these feature vectors. As illustrated in Slide \ref{fig:chapter2_nn_tfidf}, querying for a specific paper returns its most similar counterparts based on their semantic content.

This example highlights the flexibility of Nearest Neighbor and related algorithms, such as K-means clustering, in handling diverse data types. By tailoring the distance metric to the nature of the data, we can unlock meaningful insights across domains.
...
For smaller datasets, \textbf{k-fold cross-validation} is often preferred. The dataset is split into \(k\) folds, and each fold is used as a validation set while the remaining \(k-1\) folds are used for training. The process repeats \(k\) times, and results are averaged to evaluate performance. This method offers a robust mechanism for hyperparameter selection, as demonstrated in Slide \ref{fig:chapter2_cross_validation}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_2/Slide_77.jpg}
	\caption{Cross-validation accuracy for different values of \(k\). Each dot represents a trial, and the mean is represented by the line. Here we peek $k=7$ as the mean is highest in this case.}
	\label{fig:chapter2_cross_validation}
\end{figure}

While cross-validation is computationally feasible for small datasets, it becomes impractical for large datasets and complex deep learning models, and thus is not common in evaluation of practical vision algorithms.
...
Faster or approximate versions of Nearest Neighbor exist, as highlighted in Slide \ref{fig:chapter2_nn_speedup}, which make use of spatial data structures like KD-trees to accelerate the search process.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_2/Slide_49.jpg}
	\caption{Approximation methods for accelerating Nearest Neighbor search.}
	\label{fig:chapter2_nn_speedup}
\end{figure}
...
The Nearest Neighbor classifier has an intriguing theoretical property: with infinite training samples, it can approximate any function. This property arises from the ability to memorize and interpolate the training data. As the number of training points increases, the decision boundaries become increasingly fine-grained, capturing more nuanced patterns in the data.
...
\begin{figure}[H]
	...
	\caption{The curse of dimensionality: limitations of Nearest Neighbor in high-dimensional spaces.}
