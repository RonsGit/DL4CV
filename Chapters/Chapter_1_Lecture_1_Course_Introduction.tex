...
%----------------------------------------------------------------------------------------
This chapter presents my detailed lecture notes, designed to complement the material from the course. These notes build upon the lecture content, incorporating figures, examples, and concepts introduced by Justin Johnson in his lecture slides. 
...
\section{Core Terms in the Field}
...
\section{Why Study Deep Learning for Computer Vision?}
...
\newpage
\subsection{Hubel and Wiesel (1959): How Vision Works?}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_16.jpg}
...
\subsection{Larry Roberts (1963): From Edges to Keypoints}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_17.jpg}
...
\subsection{David Marr (1970s): From Features to a 3D Model}
...
\subsection{Recognition via Parts (1970s)}
...
\subsection{Recognition via Edge Detection (1980s)}
...
\subsection{Recognition via Grouping (1990s)}
...
\subsection{Recognition via Matching and Benchmarking (2000s)}
...
\subsection{The ImageNet Dataset and Classification Challenge}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_27.jpg}
...
\subsection{AlexNet: A Revolution in Computer Vision (2012)}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_28.jpg}
...
\subsubsection{Building on AlexNet: Evolution of CNNs and Beyond}

The success of AlexNet in 2012 catalyzed a wave of advancements in convolutional neural networks (CNNs), setting the stage for deeper architectures, new paradigms, and transformative innovations in neural network design.

\newpage
\textbf{ResNets and Deeper CNN Architectures (2015)}

While AlexNet showcased the power of deep CNNs, increasing network depth introduced the \textbf{vanishing gradient problem}, where gradients diminish as they propagate backward through the network, impeding effective training. This challenge was addressed by \textbf{Residual Networks (ResNets)}, proposed by He et al. in 2015 \cite{he2016_resnet}, which introduced \textbf{skip connections} (or residual connections). These connections allowed gradients to bypass certain layers, ensuring stable training even in networks with hundreds or thousands of layers.

ResNets demonstrated that very deep networks could achieve superior performance without overfitting, achieving state-of-the-art results in tasks like image classification and object detection. The success of ResNets established \textbf{deep residual learning} as a cornerstone of modern deep learning.

\textbf{Recurrent Neural Networks (RNNs) and LSTMs}

\textbf{RNNs} emerged as a natural choice for sequential data processing due to their ability to maintain a hidden state that evolves over time. Although introduced in the 1980s \cite{rumelhart1986_backpropagation}, their application to computer vision became prominent in the 2010s, especially for video analysis, activity recognition, and image captioning.

However, standard RNNs struggled with long-term dependencies due to the vanishing gradient problem. This limitation was overcome by \textbf{Long Short-Term Memory (LSTM)} networks, introduced by Hochreiter and Schmidhuber in the 1990s \cite{hochreiter1997_lstm}. LSTMs employ gating mechanisms to selectively retain, forget, and pass on information, enabling robust modeling of long-range temporal dependencies. Despite their success in tasks like video captioning and temporal activity recognition \cite{donahue2015_ltrcnn}, LSTMs have key drawbacks:
\begin{itemize}
	\item \textbf{Sequential Nature:} LSTMs process data sequentially, limiting parallelization and increasing computational expense.
	\item \textbf{Scalability Challenges:} As sequence lengths grow, LSTMs often struggle to generalize and capture global context.
\end{itemize}

These limitations paved the way for the development of attention mechanisms, which revolutionized the handling of sequential data.

\textbf{Vision Transformers (ViTs) and Attention Mechanisms}

The \textbf{attention mechanism}, popularized by Vaswani et al.'s seminal work \textbf{"Attention Is All You Need"} in 2017 \cite{vaswani2017_attention}, fundamentally changed how neural networks process data. Attention allows models to dynamically focus on relevant parts of the input data while ignoring irrelevant information. In the context of sequences, attention computes the relationship (or relevance) between tokens, assigning higher weights to important elements.

\textbf{Self-attention}, a specific form of attention, operates within a single sequence by relating each token to every other token. It uses three key components:
\begin{itemize}
	\item \textbf{Queries (Q):} Represent the current token of interest.
	\item \textbf{Keys (K) and Values (V):} Represent other tokens in the sequence.
\end{itemize}
The relevance of a token is computed as a scaled dot product between queries and keys, weighted by the corresponding values.

Transformers, models built on the concept of self-attention, initially designed for natural language processing, were extended to computer vision tasks with the introduction of \textbf{Vision Transformers (ViTs)} \cite{vit2020_transformers}. ViTs divide an image into patches, treating each patch as a token in a sequence. Self-attention mechanisms are then used to capture global dependencies across the image.

ViTs demonstrated state-of-the-art performance in image classification, object detection, and segmentation, overcoming limitations of CNNs:
\begin{itemize}
	\item \textbf{Global Context:} Self-attention enables the model to capture long-range dependencies efficiently.
	\item \textbf{Scalability:} Transformers are highly parallelizable, making them suitable for large-scale datasets and models.
\end{itemize}

However, Transformers have their own limitations:
\begin{itemize}
	\item \textbf{Memory Intensity:} Self-attention scales quadratically with the sequence length, making it computationally expensive for high-resolution images.
	\item \textbf{Data Dependency:} Transformers often require massive datasets to generalize effectively, unlike CNNs that perform well with smaller datasets.
\end{itemize}

\textbf{MAMBA: Linear-Time Sequence Modeling with Selective State Spaces (2022)}

\textbf{MAMBA} \cite{mamba2023_selective} introduces a new paradigm in sequence modeling by leveraging structured state space models (SSMs) for efficient long-range dependency learning. Unlike traditional self-attention, MAMBA achieves linear-time complexity, making it a scalable alternative for high-dimensional sequential data. This approach improves computational efficiency while retaining the ability to capture complex dependencies, making it particularly useful for vision-based sequence modeling and multimodal learning.

\textbf{Foundation Models: From Vision to Multimodal Intelligence}

The progression from AlexNet has led to the rise of \textbf{foundation models}, which integrate vision, language, and multimodal capabilities. Notable examples include:
\begin{itemize}
	\item \textbf{DINO (2021):} Self-supervised learning using Vision Transformers to learn robust representations without labels \cite{dino2021_selfsupervised}.
	\item \textbf{CLIP (2021):} Aligning vision and language embeddings for cross-modal understanding and zero-shot classification \cite{clip2021_multimodal}.
	\item \textbf{Segment Anything Model (SAM) (2023):} Generalizing image segmentation across diverse datasets \cite{sam2023_segmentation}.
	\item \textbf{Flamingo (2022):} Visual language models combining vision and language reasoning for tasks like visual question answering \cite{flamingo2022_fewshot}.
\end{itemize}

\textbf{Conclusion}

The legacy of AlexNet extends far beyond its initial success, shaping the evolution of neural network architectures and driving breakthroughs in computer vision. From CNNs to attention-based models like ViTs, and from vision-specific tasks to multimodal intelligence, this journey reflects the field's relentless pursuit of scalable, adaptable, and powerful algorithms.

\section{Milestones in the Evolution of Learning in Computer Vision}
...
\subsection{The AI Winter and Multilayer Perceptrons (1969)}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_30.jpg}
...
\subsection{The Neocognitron (1980)}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_32.jpg}
...
\subsection{Backpropagation and the Revival of Neural Networks (1986)}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_33.jpg}
	\caption{Backpropagation algorithm by Rumelhart et al., pivotal for training DNNs \cite{rumelhart1986_backpropagation}.}
	\label{fig:chapter1_backprop}
\end{figure}

\subsection{LeNet and the Emergence of Convolutional Networks (1998)}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{Figures/Chapter_1/Slide_35.jpg}
	\caption{The 2000s: Advances in hardware and algorithms enabling deep learning.}
	\label{fig:chapter1_dl_2000s}
\end{figure}

\subsection{Deep Learning Explosion (2007-2020)}
...
Recent advancements, such as the \textbf{Segment Anything Model (SAM)} \cite{sam2023_segmentation}, \textbf{DINO} \cite{dino2021_selfsupervised}, and \textbf{MAMBA} \cite{mamba2023_selective}, exemplify the cutting edge in computer vision. These models were trained on massive amounts of data over extended periods, leveraging vast computational resources to learn robust and generalizable representations. 

Their foundational nature lies in their versatility: they can be applied across a wide range of tasks without fine-tuning (or at the very least, minimal fine-tuning), making them highly effective out-of-the-box solutions. 

\newpage

By integrating self-supervised learning, multimodal reasoning, and dynamic scene analysis, these models set new benchmarks in performance and adaptability. Their ability to generalize from diverse and extensive training data makes them a powerful starting point for solving both standard and novel real-world problems.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_1/Slide_46.jpg}
	\caption{Another example for a peach-shaped armchair, generated by DALL-E \cite{dalle2021_texttoimage}.}
	\label{fig:dalle_peach}
\end{figure}
...
\newpage
\section{Key Challenges in CV and Future Directions}
