...
\begin{itemize}
	...
	\item \textbf{Improved Scalability:} Transformers naturally handle long-range dependencies and complex object relationships, making them well-suited for large-scale datasets, high-resolution images.
...
Although not covered in the main content of this chapter, we included an \textbf{enrichment section on YOLO}, a widely used object detector known for its real-time speed. YOLO differs from traditional object detectors by treating detection as a global regression problem, predicting bounding boxes and class probabilities in a single forward pass. Over successive versions, YOLO has refined its accuracy and efficiency, making it a popular choice for applications requiring real-time object detection.

\paragraph{Looking Ahead: Transformers and SOTA Detectors}
While this chapter focused on CNN-based object detectors, modern detection frameworks have evolved further with \textbf{transformer-based architectures}. Models like \textbf{DEtection TRansformer (DeTR)} and self-supervised approaches such as \textbf{DINOv2} have redefined object detection by eliminating region proposal mechanisms and relying on attention-based feature learning. 

We have yet to cover these cutting-edge techniques, but as we progress in this document, we will explore several examples of \textbf{state-of-the-art (SOTA)} object detectors that leverage transformers to push the boundaries of accuracy and efficiency.

\paragraph{Summary}
The evolution of object detectors has followed a clear trajectory:
\begin{itemize}
	\item \textbf{R-CNN} introduced region-based detection but was computationally expensive.
	\item \textbf{Fast R-CNN} improved efficiency with shared feature computation but still relied on slow region proposals.
	\item \textbf{Faster R-CNN} replaced manual proposals with a learnable \textbf{RPN}, making detection fully end-to-end.
	\item \textbf{FPNs} enhanced multi-scale detection, improving performance on small objects.
	\item \textbf{RetinaNet} was the first one-stage detector to outperform two-stage models, enabled by \textbf{Focal Loss}.
	\item \textbf{FCOS} demonstrated the viability of \textbf{anchor-free detection}, further simplifying object detection.
	\item \textbf{YOLO} has remained a widely adopted real-time detector, continuously evolving across versions.
	\item \textbf{Transformers are emerging as the next frontier} in object detection, removing the need for proposal-based architectures.
\end{itemize}

Each of these advancements has built upon prior innovations, gradually refining object detection into the highly efficient, accurate, and scalable frameworks we see today. As we move forward in this document, we will explore more advanced architectures, particularly those utilizing transformers, to understand how object detection continues to evolve.
