...
\begin{enrichment}[Additive Margin Softmax (AM-Softmax)][subsection]
	...
	\paragraph{Why angles on a hypersphere}
	Consider the simplest setting: a \emph{binary} linear classifier whose last layer compares a feature $x\!\in\!\mathbb{R}^D$ to two class prototypes (weight vectors) $w_1,w_2\!\in\!\mathbb{R}^D$. To prevent raw magnitude from dominating decisions, it is standard to $\ell_2$-normalize both features and weights onto the unit hypersphere, yielding $\tilde{x}=x/\|x\|_2$ and $\tilde{w}_j=w_j/\|w_j\|_2$. The class score then becomes the \emph{cosine} of the angle $\theta_j$ between $\tilde{x}$ and $\tilde{w}_j$, rescaled by $s>0$:
	\begin{equation}
		z_j \;=\; s\,\tilde{w}_j^\top \tilde{x} \;=\; s\,\cos\theta_j, 
		\qquad 
		\theta_j \in [0^\circ,180^\circ],\quad 
		\cos\theta_j=\tilde{w}_j^\top \tilde{x}.
		\label{eq:chapter3_amsoftmax_angles}
	\end{equation}
	Angles are the natural geometry on the hypersphere: $\theta_j{=}0^\circ$ means perfect alignment with class $j$; $\theta_j{=}90^\circ$ means orthogonality; larger $\theta_j$ indicates increasing misalignment. With conventional softmax on \eqref{eq:chapter3_amsoftmax_angles}, the binary decision boundary satisfies $\cos\theta_1=\cos\theta_2$—the \emph{angular bisector}. A sample that is only infinitesimally closer (in angle) to $w_1$ than to $w_2$ is already considered correct for class 1. This promotes separability but provides no explicit \emph{safety buffer} between classes; small perturbations can flip decisions near the bisector.
	
	\paragraph{A-Softmax (SphereFace): multiplicative angular margin and its pitfalls}
	A-Softmax (SphereFace) \cite{liu2017_sphereface} tries to make correct predictions “win by more” by tightening the angular region around the true class. It does so by \emph{multiplying} the target angle before turning it into a logit, while leaving non-target logits unchanged:
	\[
	z_y^{\text{A-Softmax}} = s\,\psi(\theta_y),\qquad z_j = s\,\cos\theta_j\ \ (j\neq y).
	\]
	Ideally one would set $\psi(\theta)=\cos(m\theta)$ with an integer $m\!\ge\!1$, but $\cos(m\theta)$ oscillates and is not monotone on $[0,\pi]$. To keep the target logit decreasing with angle (so “more aligned” always means “higher score”), SphereFace uses a \emph{piecewise monotone} surrogate:
	\begin{equation}
		\psi(\theta) \;=\; (-1)^k \cos(m\theta)\;-\;2k,\qquad \theta\in\Big[\tfrac{k\pi}{m},\tfrac{(k+1)\pi}{m}\Big],\ k=0,\dots,m\!-\!1,
		\label{eq:chapter3_amsoftmax_asf_piecewise_recap}
	\end{equation}
	and smooths the start of training with a simple blend
	\begin{equation}
		\psi_\lambda(\theta) \;=\; \frac{\lambda\,\cos\theta + \psi(\theta)}{\lambda+1},\qquad \text{with }\lambda\text{ large at first and annealed to }0.
		\label{eq:chapter3_amsoftmax_asf_lambda_recap}
	\end{equation}
	
	\textbf{Why blending with $\boldsymbol{\lambda}$ helps early on.} At random initialization, feature–prototype angles wander widely and often cross the sector boundaries where the piecewise rule in \eqref{eq:chapter3_amsoftmax_asf_piecewise_recap} changes formula. The raw multiplicative target has steep, regime-dependent slopes (its derivative scales with $m$ and flips across sectors), which can produce abrupt gradients. Taking $\lambda$ large makes $\psi_\lambda(\theta)\!\approx\!\cos\theta$: training begins with the smooth, well-behaved softmax target, and the angular margin is introduced \emph{gradually} as $\lambda\to 0$. This avoids sudden shape changes in the target curve and keeps the optimization stable while features start to align.
	
	\textbf{What the multiplicative margin does—and why it is angle dependent.} The decision condition $s\,\psi(\theta_y) > s\,\cos\theta_j$ implies (ignoring piecewise sign corrections) $\cos(m\theta_y) > \cos\theta_j$. In the range where both sides decrease with angle, this is roughly $m\theta_y < \theta_j$. Compared with softmax’s $\theta_y < \theta_j$, the \emph{allowable} angle to the correct prototype is reduced: for a fixed impostor angle $\theta_j$, the largest permitted $\theta_y$ shrinks to about $\theta_j/m$.
	
	\newpage
	
	In other words, the acceptance cone around the true class is \emph{narrowed}. Crucially, this narrowing is \emph{non-uniform}: when $\theta_y$ is already small (easy examples), the extra requirement barely changes anything; when $\theta_y$ is large (hard, near-boundary examples), the requirement becomes much stricter. That uneven pressure—combined with the piecewise definition—can lead to unbalanced gradients (easy points get little additional pull, hard points get a lot), making clusters less uniformly compact and adding tuning burden for $m$ and the blending schedule.
	
	\paragraph{A-Softmax in detail: multiplicative transform $m$ and blending $\lambda$}
	SphereFace specifies the correct-class target with a piecewise monotone mapping $\psi(\theta)$ that behaves like $\cos(m\theta)$, using the recap forms
	\begin{equation}
		\psi(\theta)\;=\;(-1)^k\cos(m\theta)-2k,\quad \theta\in\Big[\tfrac{k\pi}{m},\tfrac{(k+1)\pi}{m}\Big],\ k\in\{0,\dots,m-1\}
		\label{eq:chapter3_amsoftmax_asf_piecewise_recap_formula}
	\end{equation}
	and an optional stabilized blend
	\begin{equation}
		\psi_\lambda(\theta)\;=\;\frac{\lambda\,\cos\theta+\psi(\theta)}{\lambda+1},\qquad \lambda\ \text{large at start, annealed}\ \lambda\!\to\!0.
		\label{eq:chapter3_amsoftmax_asf_lambda_recap_formula}
	\end{equation}
	\textbf{Effect of $\boldsymbol{m}$.} Increasing $m$ narrows the acceptance cone around the correct prototype: for a fixed impostor angle $\theta_j$, the largest admissible target angle contracts from $\theta_y<\theta_j$ (softmax) to approximately $\theta_y<\theta_j/m$ (SphereFace). Geometrically, this is a “win by a percentage’’ rule whose strictness scales with $m$. Analytically, the target slope steepens because $\tfrac{d}{d\theta}\cos(m\theta)=-m\sin(m\theta)$, so around typical training angles the logit landscape is $m$ times sharper. The benefit is stronger separation; the cost is higher curvature and greater sensitivity to small angular changes.
	
	\textbf{Role of $\boldsymbol{\lambda}$.} The blend
	\[
	\psi_\lambda(\theta)=\frac{\lambda\cos\theta+\psi(\theta)}{\lambda+1}
	\]
	(see \eqref{eq:chapter3_amsoftmax_asf_lambda_recap}) starts training close to plain softmax ($\lambda$ large) and \emph{gradually} reveals the multiplicative margin as $\lambda\!\to\!0$. This staging smooths the target curve early on, reduces abrupt slope changes near sector boundaries, and prevents large, regime-dependent gradient spikes while features and weights are still unstructured.
	
	\textbf{Why the angle dependence is challenging.} The effective buffer is not fixed; it depends on the absolute angles. In practice this causes three issues. First, \emph{non-uniform pressure}: easy examples (small $\theta_y$) satisfy $m\theta_y<\theta_j$ almost automatically and receive little extra pull, whereas hard examples (large $\theta_y$) are pushed very strongly, yielding unbalanced updates within a batch. Second, \emph{gradient concentration}: the target slope magnitude scales with $m$ and varies with $\theta_y$, so a few near-boundary samples dominate the step, which can slow or destabilize convergence. Third, \emph{inconsistent geometric gaps}: two examples with the same cosine lead $\cos\theta_y-\cos\theta_j$ can face different acceptance conditions depending on their absolute angles, making cluster compactness less uniform. Together with the piecewise construction (sector switches introduce kinks where formulas change), this angle dependence adds tuning burden for the integer $m$ and the annealing schedule $\lambda$.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.70\textwidth]{Figures/Chapter_3/AMSoftmax_angular_comparison.jpg}
		\caption{Target-logit $\psi(\theta)$ for conventional softmax, A-Softmax with several $(m,\lambda)$, and AM-Softmax with $m=0.35$. AM-Softmax produces a smooth, strictly monotone target on $[0^\circ,90^\circ]$, which covers most practical feature–prototype angles. Figure courtesy of the AM-Softmax authors \cite{wang2018_amsoftmax}.}
		\label{fig:chapter3_amsoftmax_psi}
	\end{figure}
	
	\subsubsection{Method}
	\label{subsec:chapter3_amsoftmax_method_refined}
	
	\paragraph{Normalization and rescaling}
	We operate on the unit hypersphere and control confidence with a scalar $s$:
	\begin{equation}
		\tilde{x}=\frac{x}{\|x\|_2},\quad
		\tilde{w}_j=\frac{w_j}{\|w_j\|_2},\quad
		\cos\theta_j=\tilde{w}_j^\top\tilde{x},\quad
		z_j^{\text{softmax}} = s\,\cos\theta_j
		\label{eq:chapter3_amsoftmax_norm_scale_refined}
	\end{equation}
	Normalization makes decisions depend on \emph{direction} rather than magnitude; $s$ sharpens posteriors and scales gradients but does not alter the AM-Softmax boundary for a fixed $m$.
	
	\paragraph{AM-Softmax in detail: subtracting a fixed cosine margin $\boldsymbol{m}$}
	AM-Softmax changes only the correct-class logit by a constant cosine penalty,
	\begin{equation}
		z_y^{\text{AM-Softmax}} = s\,(\cos\theta_y - m),\qquad
		z_j = s\,\cos\theta_j\ \ (j\neq y),
		\label{eq:chapter3_amsoftmax_additive}
	\end{equation}
	with $m>0$ and a global scale $s>0$. In the binary normalized case, the softmax bisector $\cos\theta_1=\cos\theta_2$ shifts to
	\begin{equation}
		\cos\theta_1 - m = \cos\theta_2
		\quad\Longleftrightarrow\quad
		\cos\theta_1 - \cos\theta_2 = m,
		\label{eq:chapter3_amsoftmax_boundary}
	\end{equation}
	meaning class 1 must win by a \emph{fixed} cosine lead $m$ independent of $\theta_y$. The target $s(\cos\theta_y - m)$ is smooth and strictly monotone over $[0^\circ,90^\circ]$ and introduces no kinks or schedules; only $(s,m)$ need to be tuned. This uniform buffer tightens class cones consistently and enlarges inter-class gaps.
	...
	Across LFW, BLUFR, and MegaFace, AM-Softmax (with moderate $m$) consistently improves over plain softmax and is competitive with—often simpler to tune than—A-Softmax. Margins in the range $m\!\in\![0.30,0.45]$ balance compactness and stability; excessively large $m$ can over-tighten decision regions and slow convergence. Results reported by \cite{wang2018_amsoftmax} are summarized in Table~\ref{tab:chapter3_amsoftmax_main}. Identification and verification curves on MegaFace are shown in the below CMC figure. When considering absolute numbers, note backbone depth differences called out by the authors.
