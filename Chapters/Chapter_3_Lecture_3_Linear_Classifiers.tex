...
Linear classifiers are a cornerstone of machine learning and represent one of the most fundamental building blocks for modern neural networks. 

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_3/Slide_7.jpg}
	\caption{Neural networks are constructed from stacked building blocks, much like Lego blocks. Linear classifiers form one of the basic building blocks.}
	\label{fig:chapter3_lego_blocks}
\end{figure}

As illustrated in Slide \ref{fig:chapter3_lego_blocks}, neural networks can be viewed as systems built by stacking basic components, much like Lego blocks. Linear classifiers serve as one of these foundational blocks, naturally extending into more complex architectures, such as neural networks and convolutional neural networks (CNNs). Their simplicity belies their importance, as they enable a structured approach to mapping raw input data to class scores through a parametric framework.

In this chapter, we will explore linear classifiers in detail, beginning with their algebraic, geometric, and visual interpretations to fully understand their properties. The approach of linear classifiers consists of two key components: a \textbf{score function}, which maps raw input data to class scores, and a \textbf{loss function}, which quantifies the agreement between the predicted scores and the ground truth labels. These elements allow us to cast the problem of classification as an optimization task: minimizing the loss function with respect to the parameters of the score function.
