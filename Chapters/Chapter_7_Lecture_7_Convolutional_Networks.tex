...
\begin{itemize}
	...
	\item \textbf{Average Pooling:} Computes the average of values within each window, providing a smoothed representation of the feature map.
\end{itemize}
...
For max pooling, the gradient is passed only to the maximum value in each window; all other values receive a gradient of zero.
...
The original \(4 \times 4\) input matrix receives the following gradients:
...
Assume the output tensor of a previous layer consists of features \( x_1, x_2, \dots, x_D \), where \( D \) is the feature dimension. For each feature \( x_i \), we compute the mean and variance across the batch:
...
where \( \epsilon \) is a small constant to prevent division by zero. Normalizing in this way forces activations to have zero mean and unit variance, but such strict standardization may limit the expressiveness of the network.

To allow the model to learn more expressive transformations, BatchNorm introduces two additional learnable parameters: \( \gamma_i \) (scale) and \( \beta_i \) (shift):

\begin{equation}
	y_i = \gamma_i \hat{x}_i + \beta_i
\end{equation}

where \( \gamma_i \) allows the network to scale the normalized values, and \( \beta_i \) allows shifting them. Every feature \( x_i \) has its own \( \gamma_i \) and \( \beta_i \), making them \( D \)-dimensional vectors, where \( D \) is the number of features in the input tensor. Typically, \( \gamma \) is initialized to 1 and \( \beta \) is initialized to 0, but both are updated throughout training via backpropagation.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_7/slide_86.jpg}
	\caption{Summary of shapes and formulas of the 'Batch Normalization' process: normalization followed by learnable scaling and shifting (de-normalization).}
	\label{fig:chapter7_batchnorm_process}
\end{figure}

Why is this flexibility useful? During backpropagation, the gradient of the loss with respect to \( x_i \) is given by:

\begin{equation}
	\frac{dL}{dx_i} = \frac{\gamma_i}{\sqrt{\sigma^2_B + \epsilon}} \cdot \frac{dL}{dy_i}.
\end{equation}

Since \( \gamma \) is a learnable parameter, the network can adaptively scale the gradients, preventing them from becoming too large or too small. Additionally, the denominator \( \sqrt{\sigma^2_B + \epsilon} \) rescales the gradient based on the batch statistics, ensuring that if activations vary too much, their magnitude remains consistent, stabilizing gradient flow.

This flexibility in relaxing the harsh constraint of strict standardization (zero mean, unit variance) is often crucial for enabling deep networks to train effectively. As we will explore later, BatchNorm’s impact extends beyond just reducing internal covariate shift—it fundamentally influences gradient propagation, optimization dynamics, and network robustness.
...
\begin{enrichment}[Batch Normalization Placement][subsection]
	...
	\(\mathbf{z} = \phi(\mathbf{W}\mathbf{x} + \mathbf{b})\), should we apply Batch Normalization (BN) before or after the activation \(\phi\)? Moreover, how should Dropout be ordered relative to BN?
...
\subsubsection{Alternative Normalization Methods}
...
\noindent
\paragraph{Core Idea}
Layer Normalization operates \emph{per sample}, computing mean and variance across all features/channels of a single training example. This makes it insensitive to batch size (and thus suitable for tiny or single-sample batches).
...
For an input vector \(\mathbf{x} \in \mathbb{R}^N\),
\[
\mu(\mathbf{x}) = \frac{1}{N}\sum_{i=1}^N x_i,
\quad
\sigma^2(\mathbf{x})
= \frac{1}{N}\sum_{i=1}^N \bigl(x_i - \mu(\mathbf{x})\bigr)^2.
\]
Then LN transforms each component \(x_i\) to:
\[
y_i = \gamma_i\,\frac{x_i - \mu(\mathbf{x})}{\sqrt{\sigma^2(\mathbf{x}) + \epsilon}} + \beta_i,\quad i=1,\dots,N,
\]
where \(\{\gamma_i\}\) and \(\{\beta_i\}\) are trainable parameters (per feature), and \(\epsilon\) is a small constant for numerical stability.
...
\noindent
\textbf{Core Idea.}
Layer Normalization (LN) operates \emph{per sample}, computing mean and variance \emph{within a single training example} rather than across a batch. Unlike BatchNorm, LN thus remains effective when the batch size is very small or even one.
...
Given an input vector \(\mathbf{x} \in \mathbb{R}^N\):
\[
\mu(\mathbf{x}) 
...
= \frac{1}{N}\sum_{i=1}^N \bigl(x_i - \mu(\mathbf{x})\bigr)^2.
\]
Each component is normalized and then scaled/shifted:
\[
y_i 
= \gamma_i \,\frac{x_i - \mu(\mathbf{x})}
{\sqrt{\sigma^2(\mathbf{x})+\epsilon}}
\;+\;
\beta_i,
\quad
i=1,\dots,N,
\]
where \(\{\gamma_i\}\) and \(\{\beta_i\}\) are learnable parameters. Here, LN does not rely on batch-level statistics, making it robust for small-batch or sequence-based scenarios.

\paragraph{Extension to Convolutional Layers}

Layer Normalization can be applied to convolutional layers by computing the mean and variance across all channels and spatial positions within each individual sample:

\begin{equation}
	\mu_{n} = \frac{1}{C H W} \sum_{i=1}^{C} \sum_{j=1}^{H} \sum_{k=1}^{W} x_{n i j k},
	\quad
	\sigma_{n}^2 = \frac{1}{C H W} \sum_{i=1}^{C} \sum_{j=1}^{H} \sum_{k=1}^{W} \bigl(x_{n i j k}-\mu_{n}\bigr)^2.
\end{equation}

Each activation \(x_{n i j k}\) is then normalized as:

\begin{equation}
	\hat{x}_{n i j k} = \frac{x_{n i j k}-\mu_{n}}{\sqrt{\sigma_{n}^2+\epsilon}},
\end{equation}

followed by learnable scale \(\gamma\) and shift \(\beta\), maintaining the network’s expressiveness.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Figures/Chapter_7/layer_norm.jpg}
	\caption{Visualization of Layer Normalization operation, indicating how it is applied to each input image in the batch, with its own corresponding mean and std values \cite{becominghuman2018_allaboutnorm}.}
	\label{fig:chapter7_layernorm_visual}
\end{figure}

\paragraph{Interpretation}
\begin{itemize}
	\item \textbf{Per-Sample Aggregation:} All features (channels) and spatial positions in the \emph{same} image share the same mean \(\mu_n\) and variance \(\sigma_n^2\), ensuring consistency across different feature maps.
	\item \textbf{Batch-Size Independence:} No averaging across different samples, making LN robust for small-batch or sequence-oriented settings (e.g., RNNs, Transformers).
\end{itemize}

\paragraph{Advantages of LN}
\begin{enumerate}
	\item \textbf{Stable with Small Batches:}
	LN relies solely on per-example statistics, preserving effectiveness even with batch size \(=1\). This is particularly valuable in online learning and reinforcement learning.
	\item \textbf{Widely Used in RNNs \& Transformers:}
	Because LN does not average across batches, it naturally suits sequence models. Modern transformer architectures apply LN after multi-head attention and feedforward blocks, enhancing stability and training speed.
\end{enumerate}
...
\newpage
\begin{enrichment}[BatchNorm and \(L_2\) Regularization][subsection]
	\label{enrichment:bn_l2_regularization}
	
	\paragraph{Context and Motivation}
	Batch Normalization (BN) and \(L_2\) regularization (a.k.a.\ weight decay) are both common in modern deep learning. BN stabilizes training by normalizing feature activations in each mini-batch, while \(L_2\) shrinks weights to control overfitting. However, \emph{once BN is applied to a convolutional layer, the usual “shrink the filter weights” mechanism of \(L_2\) may change meaningfully}.
	
	\paragraph{Why Combine Them?}
	\begin{itemize}
		\item \textbf{Mild Extra Regularization:}
		BN somewhat regularizes by making activations less sensitive to weight scale; layering on \(L_2\) can further suppress overfitting.
		\item \textbf{Smoother Activations + Weight Decay:}
		BN ensures stable feature scales, which can complement the incremental shrinkage from \(L_2\), often improving convergence.
	\end{itemize}
	
	\paragraph{Key Interaction: BN Masks the Scale of Weights}
	\subparagraph{Invariance to Weight Scaling}
	BN’s mean/variance normalization effectively discards the absolute magnitude of incoming weights. As a result:
	\[
	\text{If } w \mapsto \lambda\, w,\quad \text{then }\, \mathrm{BN}(w) \approx \mathrm{BN}(\lambda\,w).
	\]
	Hence, weight decay’s “shrink the weights” effect no longer reduces overfitting in a straightforward sense, because BN can \emph{undo} that scale change.
	
	\subparagraph{Shifting Role of \(L_2\)}
	Rather than acting as a direct \emph{regularizer} on that layer’s capacity, \(L_2\) can become a mechanism controlling the \emph{effective learning rate}. When weights grow large, BN normalizes them, causing gradients to shrink inversely with weight magnitude. If there is no \(L_2\) to counterbalance this, the net effect can lead to an unbounded weight scale (and a diminishing step size), effectively making training freeze or behave oddly. With weight decay, however, the network is forced to maintain a consistent weight scale, preserving a stable gradient regime.
	
	\paragraph{Practical Pitfalls}
	\subparagraph{(1) Excluding \(\gamma,\beta\) from Decay}
	BN layers introduce \(\gamma\) (scale) and \(\beta\) (shift). Standard practice excludes these from weight decay:
	\begin{itemize}
		\item \textbf{Why Exclude?} 
		Penalizing \(\gamma\) can prevent BN from effectively rescaling channels.
		\item \textbf{Danger of Huge \(\gamma\):} 
		Conversely, if main filter weights are heavily decayed, BN might “compensate” by inflating \(\gamma\). Carefully observe or limit \(\gamma\) to avoid large amplitude in activations.
	\end{itemize}
	
	\subparagraph{(2) Small Batches}
	BN relies on stable batch-level statistics. With small batch sizes (e.g., \(<8\) or \(<16\)), the noise in mean/variance estimates can conflict with steady weight decay, causing:
	\begin{itemize}
		\item \textbf{Oscillating Updates:} Variation in BN’s normalization plus constant shrinking of weights can destabilize training.
		\item \textbf{Under/Overfitting Mix:} The network may converge suboptimally or require more complex tuning (e.g., smaller momentum in BN).
	\end{itemize}
	
	\paragraph{Recommendations}
	\begin{itemize}
		\item \textbf{Exclude or Lightly Penalize BN’s \(\gamma,\beta\):}
		Typically, BN’s scale/shift are not included in weight decay. Or use only a gentle decay on \(\gamma\).
		\item \textbf{Tune Decay Strength Carefully:}
		If BN already helps stabilize/regularize, an overly large \(L_2\) might hamper learning.
		\item \textbf{Track \(\gamma\) Behavior:}
		If \(\gamma\) grows excessively, consider lowering its learning rate or mild penalty.
		\item \textbf{For Small Batches:}
		Higher BN momentum (less variance in mean/var estimates) or alternative normalizations can reduce instability.
	\end{itemize}
	
	\paragraph{Summary}
	When a convolutional layer feeds into BN, scaling down its weights via \(L_2\) no longer directly reduces overfitting—BN can “cancel out” the shrinkage. Instead, the main effect of \(L_2\) is to bound weight scale and thus preserve a stable gradient magnitude. In many architectures, this indirect role still benefits training, but practitioners should be aware of potential pitfalls such as runaway \(\gamma\) and fragile updates with tiny batch sizes.
