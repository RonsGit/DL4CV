...
Whereas ShuffleNetV1 relied on group convolutions and channel shuffling to optimize FLOPs, \textbf{ShuffleNetV2}~\cite{ma2018_shufflenetv2} reorients the focus toward \emph{real-world efficiency}. This is driven by the observation that FLOPs alone do not adequately reflect performance on edge devices. Instead, factors such as \textbf{memory access cost (MAC) - note, this is not Multiply \& Accumulate!} and parallelism on modern hardware accelerators often determine practical inference speed. ShuffleNetV2 introduces a set of design rules that systematically address these constraints, offering a hardware-friendly approach.
...
\subsubsection{How Should We Scale a Model}
...
\subsubsection{How EfficientNet Works}
...
\subsubsection{Why is EfficientNet More Effective}
...
\subsubsection{Limitations of EfficientNet}
...
\subsection{EfficientNet-Lite Optimizing EfficientNet for Edge Devices}
\label{subsec:efficientnet_lite}

\subsubsection{Motivation for EfficientNet-Lite}
...
\subsubsection{EfficientNet-Lite Architecture}
...
\subsubsection{Performance and Comparison with Other Models}
...
\newpage
\subsection{EfficientNetV2: Faster Training and Improved Efficiency}
\label{subsec:efficientnetv2}

\subsubsection{Motivation for EfficientNetV2}
...
\subsubsection{Fused-MBConv: Improving Early Layers}
...
\subsubsection{Progressive Learning: Efficient Training with Smaller Images}
...
\subsubsection{FixRes: Addressing Train-Test Resolution Discrepancy}
...
\subsubsection{Non-Uniform Scaling for Improved Efficiency}
...
\subsubsection{EfficientNetV2 Architecture}
...
\subsubsection{EfficientNetV2 vs. EfficientNetV1}
...
\subsubsection{EfficientNetV2 vs. Other Models}
...
\newpage
\subsection{NFNets: Normalizer-Free ResNets}
\label{subsec:nfnets}

\subsubsection{Motivation: Why Do We Need NFNets?}
...
\subsubsection{Variance Explosion Without BatchNorm}
...
\subsubsection{Why Not Rescale the Residual Branch?}
...
\subsubsection{NFNets: Weight Normalization Instead of BN}
...
\subsubsection{NFNets Architecture and ResNet-D}
...
\subsubsection{Comparison Across Diverse Architectures}
...
\newpage
\subsection{Revisiting ResNets: Improved Training and Scaling Strategies}
...
\subsubsection{Training Enhancements for ResNets}
...
\subsubsection{Scaling ResNets for Efficient Training}
...
\subsubsection{ResNet-RS vs. EfficientNet: A Re-Evaluation}
...
\subsection{RegNets: Network Design Spaces}
...
\subsubsection{RegNet Architecture}
...
\begin{figure}[H]
	...
	\caption{\textbf{RegNet architecture}: A simple backbone with a stem, four-stage body, and head. Each stage consists of multiple blocks with defined parameters. These building blocks will allows us to construct architectures at different sizes that are quite efficient, while producing competitive accuracy.}
...
\subsubsection{Optimizing the Design Space}
...
\newpage
\subsection{Summary of Efficient Network Architectures}
\label{subsec:chapter11_summary}

We have surveyed a wide range of architectural innovations aimed at improving \emph{both} theoretical and real-world efficiency in deep learning. Below is an overarching summary of the progression and the main lessons learned.

\subsubsection{Grouped Convolution and ResNeXt}
ResNeXt built on the ResNet design by introducing \emph{grouped convolutions} inside its bottleneck blocks, allowing multiple parallel transformations at reduced computational cost. This approach demonstrated that FLOPs could be redistributed more effectively through grouped convolutions, improving accuracy under a fixed compute budget.

\subsubsection{Squeeze-and-Excitation (SE) Blocks}
SE blocks introduced \emph{channel-wise attention}, reweighting each feature map via a learned global context. By minimally increasing parameters and compute, SE blocks significantly boosted accuracy. Modern designs such as MobileNetV3, EfficientNet, and some RegNet variants incorporate this channel-attention mechanism to enhance representational power.

\subsubsection{MobileNet: Depthwise Separable Convolutions}
MobileNetV1 popularized \emph{depthwise separable convolutions}, splitting standard convolutions into a spatial depthwise step and a pointwise step. This drastically lowered FLOPs while retaining competitive accuracy. ShuffleNet refined the idea with \emph{channel shuffle} and grouped $1\times1$ layers, though real-world speed gains sometimes lagged behind theoretical FLOPs.

\subsubsection{MobileNetV2 and the Inverted Residual Block}
MobileNetV2 introduced \emph{inverted residual blocks} with \emph{linear bottlenecks} to mitigate activation collapse in narrow channels. This design improved accuracy and efficiency over MobileNetV1, and paved the way for subsequent refinements.

\subsubsection{NAS and MobileNetV3, ShuffleNetV2 Insights}
MobileNetV3 combined \emph{NAS}-optimized inverted bottlenecks, SE blocks, and specialized activations, achieving strong mobile efficiency. ShuffleNetV2, while focusing on memory-access cost, demonstrated hardware-friendly design rules, but generally did not match MobileNetV3’s accuracy-FLOP trade-off.

\subsubsection{EfficientNet: Compound Scaling}
EfficientNet’s \emph{compound scaling} simultaneously adjusts depth, width, and resolution, yielding state-of-the-art accuracy per FLOP. This systematic approach outperformed many manual or purely NAS-based methods, heralding a new paradigm of scaling strategies.

\subsubsection{EfficientNet-Lite and EfficientNetV2}
EfficientNet-Lite removed SE blocks and used ReLU6 to better suit mobile hardware constraints. Later, \emph{EfficientNetV2} adopted progressive learning, fused-MBConv blocks, and refined scaling, further improving training speed and real-world deployability.

\subsubsection{NFNets and BN-Free Training}
Normalization-Free Networks (NFNets) showed that one can train very deep residual networks without batch normalization by \emph{normalizing weights} instead of activations, mitigating the variance explosion problem. However, NFNets often require higher FLOPs or more complex weight-standardization to achieve top-tier accuracy.

\subsubsection{Revisiting ResNets and Scaling}
Well-tuned ResNets (e.g., ResNet-RS) show that older backbones can rival modern methods when trained with improved schedules, stronger data augmentation, and advanced optimizers.

\subsubsection{RegNets: Optimizing Design Spaces}
RegNets systematically explored architectural hyperparameters, finding families of networks that scale in controlled ways. This structured approach often yields strong accuracy at lower search cost than heavy NAS pipelines.

\subsubsection{Key Takeaways}
\begin{itemize}
	\item \textbf{FLOPs Are Not Everything.} Real-world latency depends greatly on operator fusion, memory-access patterns, and parallel efficiency. Many “FLOPs-saving” designs underperform in actual deployments.
	\item \textbf{NAS Is Powerful but Expensive.} Automated searches (MobileNetV3, MnasNet) can find better topologies but demand large compute resources. Simpler heuristics (compound scaling, grouped conv) can also succeed.
	\item \textbf{Hardware Constraints Vary.} Mobile-targeted designs do not always excel on GPUs or TPUs. Designers must balance platform-specific considerations (e.g., depthwise speed, memory bandwidth).
	\item \textbf{BatchNorm Is Prevalent but Not Unavoidable.} BN typically stabilizes training, but NFNets show that normalization-free techniques can still yield high accuracy, at the cost of more complex weight constraints or higher FLOPs.
	\item \textbf{Architecture Design Spaces Can Be Streamlined.} RegNets illustrate how design dimensions (depth, width, group size) can be reduced to a few continuous hyperparameters, facilitating quick iteration without extensive NAS.
	\item \textbf{Scaling Strategies, Not Just Operator Choices.} EfficientNet and EfficientNetV2 emphasize the impact of precisely scaling depth, width, and resolution. In many cases, \emph{how you scale} matters as much as \emph{which blocks} you use.
\end{itemize}

\paragraph{Conclusion}
The evolution of efficient architectures has yielded a variety of design philosophies—grouped and depthwise separable convolutions, inverted residuals, channel attention, compound scaling, BN-free training, and structured design spaces. Practitioners now have a rich toolbox to fine-tune their models for specific hardware targets, accuracy needs, or resource constraints, reaffirming that \emph{no single network is universally optimal} across all deployment scenarios.
