...
For example, as shown in Figure \ref{fig:random_search}, random search achieves an accuracy of only \( 15.5\%\) on CIFAR-10, far from the \( 95\%\) state-of-the-art performance. The impracticality of densely sampling the parameter space motivates us to explore more intelligent strategies.  
...
\begin{figure}[H]
	...
	\label{fig:gradient_steepest_directions}
\end{figure}
...
\begin{figure}[H]
	...
	\label{fig:gradient_descent_contour}
\end{figure}
...
The version of gradient descent shown in Figure \ref{fig:gradient_descent_contour} is known as \textbf{Batch Gradient Descent} or \textbf{Full Batch Gradient Descent}.
...
\begin{figure}[H]
	...
	\label{fig:high_condition_number}
\end{figure}
...
\begin{figure}[H]
	...
	\label{fig:momentum_alternative}
\end{figure}
...
\begin{figure}[H]
	...
	\label{fig:adam_hyperparams}
\end{figure}

Slide \ref{fig:adam_hyperparams} highlights the widespread adoption of Adam with default hyperparameters (\( \beta_1 = 0.9, \beta_2 = 0.999, \eta = 10^{-3} \text{ or } \eta = 10^{-4}\)) in numerous deep learning papers. These settings work well across a variety of tasks with minimal tuning. 
...
AdamW addresses several issues inherent in traditional Adam with L2 regularization:
\begin{itemize}
	\item \textbf{Consistent Regularization:} Decoupling weight decay ensures that regularization consistently penalizes parameter magnitudes.
	\item \textbf{Improved Generalization:} By treating weight decay as an independent process, AdamW achieves better generalization across diverse tasks.
	\item \textbf{Minimal Overhead:} AdamW retains the computational efficiency of Adam while enhancing its robustness and applicability.
\end{itemize}
...
\begin{figure}[H]
	...
	\label{fig:second_order_quadratic}
\end{figure}
...
\begin{figure}[H]
	...
	\label{fig:second_order_limitations}
\end{figure}
